...
import { ResultSetHeader, FieldPacket } from 'mysql2'; // Correct types from mysql2
...
export const addMeal = async (req: Request, res: Response): Promise<Response> => {
    try {
        ...
        const [addedMeal]: [Meal[]] = await db.execute('SELECT * FROM meals WHERE id = ?', [insertId]);
...
export const getMeals = async (req: Request, res: Response): Promise<Response> => {
    try {
        // Fetch meals from the database and explicitly type the result
        const [meals, _]: [Meal[], FieldPacket[]] = await db.execute('SELECT * FROM meals');
